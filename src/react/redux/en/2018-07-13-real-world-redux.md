...
All the development will be in the [reactnd_chirper_app_master](https://github.com/suabochica/react-nanodegree/tree/master/react_redux/reactnd_chirper_app_master) folder.
...
A Reducer describes _how_ application's state changes. You will often see the Object Spread Operator (`...`) used inside of a reducer because a reducer **must return a new object** instead of mutating the old state. Reducers have the following signature:
...
We will need to write an asynchronous action creator since we need to record whether the logged in user liked a tweet not only in the store but also in the database. _Redux thunks_ to the rescue!.
...
The code only update the UI once we receive confirmation that the backend update was successful. This can make the app seem laggy. A common approach to UI updates is Optimistic Updating: updating the UI _before_ the action gets recorded on the backend so it seems more performant.
