...
This post is recorded in the [commit#a5f33e8a](https://github.com/suabochica/react-nanodegree/commit/a5f33e8a06bf70b31ac5ef155268bb4713701134) with the changes we describe.

Connect & Provider
------------------

As you seen, Redux doesn't have any relation to React. React is another UI library that can leverage Redux for more predictable state management. Looking at our own implementation of React and Redux together, the real _key is that we are passing down our store as a prop and utilizing it to get the state `dispatch()` or `subscribe()` whenever we need to._ The whole goal of our abstraction should be making the store's three responsibilities work as seamlessly with react as possible.
...
Fortunately, after React v16.3 we can use the [React Context API](https://reactjs.org/docs/context.html). Context provides a way to pass data through the component tree without having to pass props down manually at every level.
