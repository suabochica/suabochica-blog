...
TODO: Definición de useRef

TODO: Explicar el ejemplo del hover
...
Para habilitar el efecto del hover sobre una image, se va a crear un componente con el nombre ~ImageToogleOnMouseOver~ al cual se delegará el uso de ~useEffect~ y utilizará dos propiedades: ~primaryImg~ y ~secondaryImg~. Cada imagen atenderá el estado por defecto y el estado del hover respectivamente. A continuación se muestran los cambios sobre la primera versión del código.
...
El componente ~ImageToggleOnMouseOver~ tendría la siguiente estructura:
...
Ahora es momento de manejar los eventos ~onMouseOver~ y ~onMouseOut~:
...
TODO: Modificar proyecto de react hooks
TODO: Conclusiones sobre useRef
