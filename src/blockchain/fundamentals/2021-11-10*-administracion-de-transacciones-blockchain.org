...
* PROGRESS Firma de una Transacción
Al comenzar con una billetera, eventualmente comenzará a realizar algunas transacciones por sí mismo. En esta sección, se retrocedera un poco a la teoría para explorar algunas ideas más sobre lo que hace que esas transacciones sean seguras. El propósito es que esto le ayude a tener la confianza para comenzar a trabajar con estas herramientas por cuenta propia.
...
Una vez la transacción es firmada va desde el emisor hasta el receptor como una mensaje de transacción. En Bitcoin este mensaje es propagado a la cadena como una _Unspent Transaction Output_ conocida como UTXO. Solo los UTXO pueden ser usado como entradas para aceptar una transacción.
