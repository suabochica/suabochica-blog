...
Afortunadamente, desde React v16.3 podemos usar la [React Context API](https://reactjs.org/docs/context.html). Context proporciona una forma de pasar datos a través del árbol de componentes sin tener que pasar props manualmente en cada nivel.
...
De manera similar, también podemos aprovechar la función `connect()` de `react-redux` directamente. `connect()` es una función de alto orden que recibe dos argumentos (además de algunos argumentos opcionales) y _devuelve una función_. Observa su firma a continuación:
...
Es más: incluso puedes ver que algunos desarrolladores prefieren un enfoque ["duck"](https://medium.freecodecamp.org/scaling-your-redux-app-with-ducks-6115955638be), donde los archivos de Redux y manejo de estado están completamente separados de los archivos que renderizan la UI.
